...
def dark_mode():
    ...
    if is_on:
        ...
        caption.config(background="black", foreground="white")
        label.config(background="black", foreground="white")
        answer_label.config(background="black", foreground="white")
        output_text.config(background="black", foreground="white")
        output_image.config(background="black", foreground="white")
        dark_mode_button.config(text="Light Mode")
        is_on = False
    else:
        window.config(background="white")
        caption.config(background="white", foreground="black")
        label.config(background="white", foreground="black")
        answer_label.config(background="white", foreground="black")
        output_text.config(background="white", foreground="black")
        output_image.config(background="white", foreground="black")
...
# Caption
caption = Label(window, text="Dice Roller", font=("arial", 30, "bold"), background="white")
caption.pack()

# "Enter your guess here" label
label = Label(window, text="Enter your guess here", font=("arial", 12), background="white")
label.pack()

# Input entry box
input_field = Entry(window, font=("arial", 10), width=5, bd=3, relief="ridge")
input_field.pack()
...
# Output text
output_text = Label(window, background="white")
output_text.place(x=110, y=180)

# Output image
output_image = Label(window, background="white")
